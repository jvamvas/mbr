...
results_file = jsonlines.open(Path(__file__).parent / f"results_{num_samples}n_by_{num_references}s_cometinho_{wmt}_{language_pair}_seed{seed_no}.jsonl", "w")
...
translations = mbr_standard_comet(
    ...
    references=[[row[i] for row in samples] for i in range(len(samples[0]))][:num_references],
