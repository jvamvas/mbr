...
for language_pair in language_pairs:
    ...
    n_by_n_translations = n_by_n_data[0]["translations"]
    ...
    for x in X:
        ...
        accuracy = np.mean([1 if a.strip() == b.strip() else 0 for a, b in zip(aggregate_translations, n_by_n_translations)])
    ...
    for x in X:
        if x == X[0]:
            ...
            accuracy = np.mean([1 if a.strip() == b.strip() else 0 for a, b in zip(n_by_s_translations, n_by_n_translations)])
