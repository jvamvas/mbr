...
if beam_results_path.exists():
    with jsonlines.open(beam_results_path) as f:
        data = list(f)
    results = {result["lang_pair"]: result[metric] for result in data if result["testset"] == testset}
...
if sampling_chrf_results_path.exists():
    with jsonlines.open(sampling_chrf_results_path) as f:
        data = list(f)
    sampling_results = {result["lang_pair"]: result[metric] for result in data if result["testset"] == testset and result["method"] == "Sampling"}
    ...
    chrf_pairwise_results = {result["lang_pair"]: result[metric] for result in data if result["testset"] == testset and result["method"] == "MBR with standard ChrF"}
    ...
    chrf_aggregate_results = {result["lang_pair"]: result[metric] for result in data if result["testset"] == testset and result["method"] == "MBR with aggregate ChrF"}
...
for lang_pair in language_pairs:
    ...
    if path.exists():
        with jsonlines.open(path) as f:
            data = list(f)
        results = {result["lang_pair"]: result[metric] for result in data if result["testset"] == testset and result["num_aggregates"] == 1024}
...
for lang_pair in language_pairs:
    ...
    if path.exists():
        with jsonlines.open(path) as f:
            data = list(f)
        results = {result["lang_pair"]: result[metric] for result in data if result["testset"] == testset and result["num_aggregates"] == 32}
        print(f"{results[lang_pair]:.1f} & ", end="")
    else:
        print("tba & ", end="")
print(r"tba / tba \\\\")
