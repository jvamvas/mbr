...
print("Total:")
for j, num_subsamples in enumerate([2**i for i in range(len(durations) - 1)]):
    print(f"S = {num_subsamples}: {durations[j]:.2f}s")

print("Average per segment:")
for j, num_subsamples in enumerate([2**i for i in range(len(durations) - 1)]):
    print(f"S = {num_subsamples}: {durations[j] / num_segments:.2f}s")
